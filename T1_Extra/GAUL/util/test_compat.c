 ...
 **********************************************************************
  ...
  test_compatibility - Test Compatibility/Portability stuff.
 ...
 **********************************************************************
  ...
  Synopsis:	Test the compatibility/portability functions.

  Updated:	13 Mar 2002 SAA	First version.
...
boolean test_ipow(void)
  ...
  for (i=0; i<4; i++)
    {
    if (result[i] != ipow(n[i], e[i])) return FALSE;
    }
 ...
 */
boolean test_memcpy(void)
  {
  char *src="abcdefghijklmnopqrstuvwxyz";
  char *dest="                          ";
  char *orig="abcdefghijklmnopqrstuvwxyz";

  if (strncmp(orig, memcpy(dest, src, 26), 26)!=0) return FALSE;
 ...
 */
boolean test_strchr(void)
  {
  char *src="abcdefghijklmnopqrstuvwxyz";

  if (strchr(src, 'a')!=&src[0]) return FALSE;
  if (strchr(src, 'm')!=&src[13]) return FALSE;
  if (strchr(src, 'X')!=NULL) return FALSE;
...
char *strrchr(const char *str, int c)
  ...
  printf("ipow()                %s  %s\n, HAVE_IPOW?"system":"compat", test_ipow?"passed":"failed");
  printf("memcpy()              %s  %s\n, HAVE_MEMCPY?"system":"compat", test_memcpy?"passed":"failed");
  printf("strchr()              %s  %s\n, HAVE_STRCHR?"system":(HAVE_INDEX?"index()":"compat"), test_strchr?"passed":"failed");
  printf("strrchr()             %s  %s\n, HAVE_STRRCHR?"system":(HAVE_RINDEX?"rindex()":"compat"), test_strrchr?"passed":"failed");
